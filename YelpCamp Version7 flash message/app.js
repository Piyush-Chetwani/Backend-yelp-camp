...
app.put("/campgrounds/:id",checkCampgroundOwnership, function(req,res){
  ...
  Campground.findByIdAndUpdate(req.params.id, checkCampgroundOwnership, function(err,updatedCampground){
    if(err){
      res.redirect("/campgrounds");
    } else {
  ...
  User.register(new User({username: req.body.username}), req.body.password, function(err,user){
    if(err){
      ...
      passport.authenticate("local")(req, res, function(){
        req.flash("success", "Registered Succesfully " + user.username);
        res.redirect("/campgrounds");
      });
    }
  });
